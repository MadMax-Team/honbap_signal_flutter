...
import 'package:honbap_signal_flutter/screens/signal/widgets/signal_send_alert_dialog.dart';
...
class _SignalListScreenState extends State<SignalListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<SignalStateBloc, SignalStateState>(
          builder: (context, state) {
        if (state.state != SignalState.signaling) {
          return const Center(
            child: Text('시그널 ON 상태에서만 조회할 수 있습니다.'),
          );
        }
        return BlocBuilder<SignalListBloc, SignalListState>(
                            ...
                            onTap: () async {
                              ...
                              if (res ?? false) {
                                showDialog(
                                  context: context,
                                  builder: (context_) {
                                    return SignalSendAlertDialog(
                                      nickName: state.signals[index].nickName!,
                                    );
                                  },
                                );
                              }
